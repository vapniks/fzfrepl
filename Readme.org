...
are used by recursive calls to fzfrepl via fzftool (alt-j/k).
...
| alt-j  | pipe output to fzftool (if available). See pipelines section below.                                     |
| alt-k  | pipe output to fzftool (if available), and quit. See pipelines section below.                           |
