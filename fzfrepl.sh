...
FZF_DEFAULT_OPTS+=" --bind 'enter:replace-query,ctrl-j:accept,ctrl-t:toggle-preview,ctrl-k:kill-line,home:top,alt-1:reload(cat ${FZFREPL_HISTORY}),alt-2:reload(cat ${FZFREPL_COMMANDS}),alt-3:reload(cat ${tmpfile2}),ctrl-s:execute-silent(if ! grep -Fqs {q} ${FZFREPL_COMMANDS};then echo {q} >> ${FZFREPL_COMMANDS};fi),alt-h:execute(eval $helpcmd1|less >/dev/tty),ctrl-h:execute(eval $helpcmd2|less >/dev/tty),ctrl-v:execute(less ${cmdinput:-${file}} >/dev/tty),alt-v:execute(eval ${(Q)cmd} ${cmdinput} | less >/dev/tty),alt-w:execute-silent(echo ${cmd}|xclip -selection clipboard)' --header='C-g=quit,C-j=finish,C-t=toggle preview,RET=accept,M-w=copy
M-v=view,C-v=view all' --preview-window=right:50% --height=100% --prompt '${prompt}' ${FZFREPL_DEFAULT_OPTS}"
